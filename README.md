...
History lists show the complete shell-quoted command, including every argument, and support `--json`. Filter with `--provider`, `--command`, `--outcome`, `--code`, `--query`, `--since`, and `--until`. IDs in the list open full diagnostics with `get`; successful history queries and completion lookups stay hidden unless you add `--include-utility`.
