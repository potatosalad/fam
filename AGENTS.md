...
- Commands: `src/cli.ts` and `src/{ancestry,myheritage,findmypast}/cli.ts`. Keep credential setup in `src/credentials.ts` and persistence in `src/storage.ts`; no implicit subprocesses or host-specific defaults.
...
- Keep CLI help and README setup examples in sync. Check `npm pack --dry-run` when changing packaging; all four commands must work outside the checkout.
