...
- Document only `fam PROVIDER COMMAND` and `npm run fam -- PROVIDER COMMAND`. Old names belong only in explicit migration/compatibility guidance, not current setup or usage examples.
