...
- Commands: `src/cli.ts` and each provider's `src/<provider>/cli.ts`. Keep credential setup in `src/credentials.ts` and persistence in `src/storage.ts`; no implicit subprocesses or host-specific defaults.
