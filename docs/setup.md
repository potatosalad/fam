...
On Windows, use an absolute Windows path for the prefix. npm puts `familysearch.cmd`, `ancestry.cmd`, and `myheritage.cmd` directly in that directory.
...
The same option works with `ancestry` and `myheritage`. It takes precedence over environment variables. The CLI preserves the password exactly and does not print it. Keep the input file private.

## Storage and profiles

All three CLIs use the same configuration root:
...
FamilySearch stores `login.json` and `session.json` at the root. Ancestry uses the `ancestry/` subdirectory; MyHeritage uses `myheritage/`. Device IDs, cookies, and pending authentication state also live here.
...
For an expired MyHeritage browser session, import a fresh HAR instead. Complete any website verification before capturing it.

## Output files

Commands print JSON unless you pass `--out FILE`. Create output directories first. Image downloads also save a JSON file with the source citation and checksum, and refuse to overwrite existing files.
