...
Search combines BM25 keyword scoring with a small, inspectable vocabulary of related concepts. It runs entirely locally, needs no profile or model download, and never executes a provider command. Results include registered commands and individual operations from the FamilySearch genealogy catalog. Operation matches identify the existing `familysearch.api call` command, prefill `--operation`, and provide operation-specific inspection and input-template commands. Provider help and `familysearch.api --help` list the available genealogy operation groups.
