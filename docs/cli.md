...
Search then automatically applies a second ranking stage with [MiniLM L6](https://huggingface.co/cross-encoder/ms-marco-MiniLM-L6-v2). It reads each query and command description, including the best matching window of up to 64 words from a supporting guide passage when present, and orders the top **100** BM25 + Arctic candidates by relevance. Results retain the full supporting excerpt and guide link. Use `--no-rerank` to skip this stage for comparison or lower latency. It runs locally with the same inference runtime and automatically downloads about 24 MB of additional pinned, quantized model/tokenizer files on first use. Subsequent searches work offline.
