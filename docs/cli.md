...
Search combines **0.20 BM25 lexical + 0.80 semantic** scores using [Snowflake Arctic Embed XS](https://huggingface.co/Snowflake/snowflake-arctic-embed-xs). It runs locally on the CPU with a quantized 384-dimensional model, requires no API key, Python, GPU, or separate service, and never executes a provider command. The first search automatically downloads about 24 MB of pinned model/tokenizer files from Hugging Face and indexes the command catalog. Later searches work offline. Query text stays on the machine; the normal [command history](#command-history) policy still applies.
