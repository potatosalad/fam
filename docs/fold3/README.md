...
`--entry-id` also accepts the original `/sub-image/ID` URL returned by entry results. Bare entry numbers are checked against the `INDEX_RECORD` namespace first: if a record also exists, the CLI refuses the ambiguous number instead of returning an unrelated entry. Use `fold3.record get --record-id ID` for an `INDEX_RECORD`, or preserve the original sub-image URL for an entry. A `/record/ID` URL is rejected as entry input. The returned entry wrapper must still match both `SUB_IMAGE` and the requested ID. Dry runs validate the URL type locally; namespace collision checks require a normal read.
