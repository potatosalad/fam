...
A recognized page can have citation and navigation metadata without a scan link. In that case, image and browse reads return `available: false`, `imageSource: null`, and `unavailableReason: "image-unavailable"`, retaining the page metadata. A download reports `IMAGE_UNAVAILABLE`.
