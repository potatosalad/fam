...
HTML search fields: person name links (`a[id^=name-]`) and non-person journal/record links (`a[id^=record-]`) use `/DB{collectionId}/r/{recordId}`, with image links under `/databases/{slug}/image/`, labeled event/value blocks, and relationships in the third cell. For person rows, `.nameValueDiv` is the collection title; journal rows put the collection title in the record link and the category in `.nameValueDiv`. IDs remain decimal strings. `.placeholder-text` identifies concealed fields. `.total-hits`, `.index-page`, and `.page-size` provide pagination, with 50 rows per page observed; an attempted `pagesize` query did not change the server page size and is not exposed. Successful empty results still contain pagination inputs. Unexpected HTML is an error.
...
## Volume browsing and export implementation

`ExtendedDropdowns.volumes` supplies allowed volume IDs and display names. The image GET supports `cId` plus `volumeId` alone to open the first page, and an optional `pageName` to jump directly without an indexed `rId`. The returned `#pages` value identifies the actual page label; `#hdnVolumeid` and `#hdnCollectionID` identify the returned scope. fam checks that they agree with the requested volume and explicit page. The `#hdnPrevPageName`/`#hdnNextPageName` and corresponding `PageRid` fields supply neighboring page links; disabled arrows represent endpoints. Names are opaque page labels. Metadata traversal uses ordinary validated image GETs and does not use the viewer's POST helpers or export their credential-bearing image payloads.

Research export is a local orchestration of the same search and record GETs. It has no provider-side export endpoint. It checkpoints atomically after every record, stores the query and detail setting, and hashes the current page's ordered IDs, total, and page size. Resume verifies that page and its saved prefix before continuing, including when the record limit stops within a page. A 500-ms pause separates search-page requests and a 250-ms pause precedes each detail lookup. Duplicate IDs, changed query/field definitions or counts/order, and inconsistent pagination halt continuation. The file is capped at 50 MiB/10,000 records, with an explicit per-run limit of 1–1,000. This catches observed checkpoint-page changes but cannot provide a transactional snapshot across an independently changing remote index.
