...
Reproduce the read-only acceptance run with `npm run verify:research`. It downloads the two originals to a fresh ignored artifact directory and emits a summary without account data or signed URLs. To save a shareable report, use `npm run verify:research -- --report report.json --label workstation`. Add `--cli` to also check the built public `familysearch` entry point from this checkout's help, schema example, download, transcript, and search from fresh processes outside the checkout.
