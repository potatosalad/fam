...
`ResearchError.code` distinguishes `access-denied`, `security-challenge`, `not-found`, `throttled`, `temporary-failure`, `unexpected-content`, `schema-change`, and `pagination`. Explicit 401 responses renew authentication once; 403/security challenges do not start login loops. Metadata/image reads make at most three total attempts for temporary connection failures and 429/502/503/504, with stderr progress and bounded backoff. `--fail-fast` disables transient retries. A server `Retry-After` exceeding the remaining cumulative 30-second backoff budget is surfaced instead of retrying early. Once a streamed image body has started, a failed transfer must be rerun; no incomplete image is kept.

Transport defaults to `auto`: HTTP unless a browser route is remembered for the origin. If that route fails because the browser service is unavailable or returns an infrastructure error, document reads retry once over HTTP. A successful HTTP response clears that origin's remembered browser preference. This includes the observed read-only image/film/waypoint metadata POST routes; mutations and verification prompts are not replayed by this recovery. Explicit `--transport browser` remains browser-only; `--fail-fast` disables HTTP recovery. Inspect routing with `fam cli.browser.transport get --provider familysearch --origin https://www.familysearch.org`; `--transport http` selects HTTP for one command.
