...
Recovered from Android 4.0.2 (138). `findagrave schema NAME` returns the complete document or route. Full GraphQL documents and source references are in [contracts.json](contracts.json).
...
Authentication/linking documents are cataloged; generic execution is reserved for non-authentication operations. Use `findagrave auth` for the native password flow.
