...
// Packing, extraction, and six cold CLI starts share this budget on slower CI runners.
test('packed documentation, search, and completion work with source files removed', {timeout: 120000}, async t => {
  const directory = await mkdtemp(join(tmpdir(), 'fam-doc-package-'));
  t.after(() => rm(directory, {recursive: true, force: true}));
  const command = async (label, file, args, options = {}) => {
    const started = performance.now();
    t.diagnostic(`Starting ${label}`);
    try {
      return await run(file, args, {...options, signal: t.signal});
    } finally {
      t.diagnostic(`${label}: ${Math.round(performance.now() - started)} ms`);
    }
  };
  // Inspect the manifest of the same archive we execute, without a second npm pack.
  const archive = firstPackage(JSON.parse((await command('npm pack', 'npm',
  ...
  await command('archive extraction', 'tar', ['-xzf', join(directory, archive.filename), '-C', directory]);
  ...
  const invoke = (...args) => command(`fam ${args.join(' ')}`, process.execPath, [join(installed, 'bin/fam.mjs'), ...args], {
