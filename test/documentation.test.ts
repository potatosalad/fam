...
test('documentation semantic search shares the command corpus, reranks sections, and falls back without losing matches', async () => {
  let corpus: string[] = [];
  ...
  const docCorpus = corpus;
  await searchCommands('relatives', {provider: 'americanancestors', rerank: false}, semantic);
  assert.deepEqual(corpus, docCorpus, 'Switching searches must not invalidate the shared embedding cache.');
