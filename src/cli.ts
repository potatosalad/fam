...
familysearch auth                    Authenticate and save the session
familysearch status                  Inspect local session status (no tokens)
familysearch credentials [--stdin]   Save login details (hidden prompt, environment, or JSON stdin)
familysearch refresh                 Renew the saved access token
familysearch verify                  Read-only live smoke test, summarized
familysearch whoami                  Read the current user
familysearch metadata                Read mobile login metadata (tokens removed)
familysearch person ID               Read a GEDCOM X person
familysearch ancestry ID [DEPTH]      Read GEDCOM X ancestors (default 2)
familysearch mobile-person ID        Read the mobile v2 person DTO
familysearch mobile-pedigree ID [DEPTH]
familysearch tree-status
familysearch get /platform/...       GET an API path (quote query strings)
familysearch ops [GROUP]             List typed genealogy operations
familysearch schema OPERATION [--example]  Show contract or correctly nested example
familysearch call OPERATION [FILE] [--input FILE|-] [--query key=value ...]
${researchHelp}

Append --out FILE to save JSON with mode 0600 instead of printing it.
Set FAMILYSEARCH_CONFIG_DIR to an absolute path to override the user config directory.
...
async function main() {
  ...
  if (!command || command === 'help' || args.includes('--help')) { console.log(help); return; }
  if (command === 'credentials') {
    if (args.length > 2 || id && id !== '--stdin') throw new Error('Use familysearch credentials [--stdin].');
    await configureCredentials('familysearch', { stdin: id === '--stdin' });
    console.log(stringifyJson({ saved: true, credentialDirectory: CREDENTIAL_DIR, next: 'familysearch auth' }, 2));
    return;
  }
  if (args.includes('--stdin')) throw new Error('--stdin belongs to familysearch credentials.');
  ...
  if (!supported.has(command)) throw new Error(`Unknown command. Use "familysearch --help".`);
  const client = await FamilySearchClient.open();
  let result: unknown;
  switch (command) {
    case 'ops': result = listOperations(id).map(({ name, method, path }) => ({ name, method, path })); break;
    case 'schema': {
      if (args.length > 3 || depth && depth !== '--example') throw new Error('Use familysearch schema OPERATION [--example].');
