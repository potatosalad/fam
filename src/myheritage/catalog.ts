...
export function graphqlOperation(name: string): GraphQLOperation {
  ...
  throw new Error(matches.length ? `Ambiguous operation ${name}; use its full ID from fam myheritage.api list.` : `Unknown operation ${name}; use fam myheritage.api list.`);
}
...
export function restOperation(name: string) {
  ...
  if (!op) throw new Error(`Unknown REST operation ${name}; use fam myheritage.api list.`);
  return op;
}
...
export function prepareRest(name: string, args: RestArguments = {}): {url: string; options: ApiRequest; anonymous?: boolean} {
  ...
  if (op.parameters.some(p => p.kind === 'Body' || p.kind === 'FieldMap') && body === undefined) throw new Error(`${name} requires body; use fam myheritage.api describe --operation ${name}.`);
  if (op.encoding === 'form') {
