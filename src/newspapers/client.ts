...
export interface SearchOptions {keyword?:string; publicationId?:string; country?:string; region?:string; city?:string; from?:string; to?:string; sort?:'score'|'date-asc'|'date-desc'; limit?:number; cursor?:string}
export function searchQuery(options: SearchOptions = {}) {
  const query: Record<string,string> = {product:'1','entity-types':'page',count:String(integer(options.limit ?? 20)),start:options.cursor === undefined ? '*' : text(options.cursor,4096)};
...
export const operations = {
  ...
  search: {method:'GET',path:'/api/search/query',input:{keyword:'string?',publicationId:'decimal ID?',country:'string?',region:'string?',city:'string?',from:'date?',to:'date?',sort:'score | date-asc | date-desc?',limit:'1–100?',cursor:'opaque nextStart?'},description:'Page search with opaque cursor pagination.'},
  ...
  ocr: {method:'GET',path:'/api/client/image/ocr/',input:{pageId:'decimal ID',articleId:'decimal ID?',clippingId:'decimal ID?',x:'integer?',y:'integer?',width:'integer?',height:'integer?'},description:'Page OCR, or OCR for a clipping or rectangular article selection; empty/unavailable text is explicit.'},
} as const;
...
export class NewspapersClient {
  ...
  search(options: SearchOptions = {}) {
    ...
    return this.run(async () => {
      ...
      return {records:publicValue(result.records),recordCount:result.recordCount,nextCursor:result.records.length ? result.nextStart || null : null,partialResults:result.partialResults === true,
        limit:Number(query.count), accessNote:'Search results do not establish subscription access to every scan.'};
    });
  }
  ...
  ocr(pageId: string, options: {articleId?:string;clippingId?:string;x?:number;y?:number;width?:number;height?:number} = {}) {
    id(pageId);if (options.articleId && options.clippingId) throw new Error('Choose an article or clipping, not both.');
    if (options.articleId) id(options.articleId);if (options.clippingId) id(options.clippingId);
    ...
    if (options.articleId && options.x === undefined) throw new Error('--article-id requires its rectangle (--x, --y, --width, --height).');
    ...
    return this.run(async () => {
      const value=await this.authorize(pageId);if (!value.image.canView || typeof value.iat !== 'string') throw new NewspapersError('access-denied');
      const selection = !options.clippingId && options.x === undefined ? {x:0,y:0,width:integer(value.image.width,1,100000),height:integer(value.image.height,1,100000)} : options;
