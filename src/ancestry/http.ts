...
export const GATEWAY = 'https://gateway.ancestry.com';
export const AUTH = 'https://auth.ancestry.com';
export const USER_AGENT = 'Ancestry/18.16.3 (Android 16)';
const origins = new Set([GATEWAY, AUTH, 'https://www.ancestry.com', 'https://mediaupload.ancestry.com']);
export function checkAncestryUrl(url: URL): void {
  if (!origins.has(url.origin) || url.username || url.password) throw new Error('Refusing a request outside the Ancestry API origins.');
}
export class AncestryHttpError extends Error {
  constructor(readonly status: number, readonly path: string, readonly retryAfter?: string) {
    super(`Ancestry HTTP ${status} from ${path}`);
  }
}
export class AncestryHttp {
  ...
  async exchange<T = unknown>(url: string | URL, options: ApiRequest = {}): Promise<ApiResponse<T>> {
    ...
    if (!response.ok) {
      await response.text();
      throw new AncestryHttpError(response.status, `${target.origin}${target.pathname}`, response.headers.get('retry-after') ?? undefined);
    }
    ...
    try { return result(parseJson(text)); } catch { throw new Error(`Expected JSON from Ancestry ${target.pathname}.`); }
  }
}
