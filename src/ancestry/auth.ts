...
export async function tokenRequest(http: AncestryHttp, fields: Record<string, string>, refresh = false): Promise<AncestryTokens> {
  ...
  if (data.must_change_password_before_next_login) throw new Error('Ancestry requires a password change. Complete it on Ancestry, then run fam ancestry.session login again.');
  if (!data.access_token || !data.refresh_token) {
    if (fields.service_provider === 'user_credentials' && typeof (data as unknown as PendingAuth).verification_token === 'string') {
      ...
      throw new Error('Ancestry requires email verification. Run fam ancestry.session login --send-code, then fam ancestry.session login --code CODE.');
    }
    throw new Error('Ancestry returned an incomplete token response; run fam ancestry.session login to sign in again.');
  }
  return data;
}
...
async function pendingAuth(): Promise<PendingAuth> {
  ...
  if (!pending?.verification_token) throw new Error('No pending Ancestry verification; run fam ancestry.session login first.');
  return pending;
}
...
export async function verifyAncestryCode(code: string): Promise<AncestrySession> {
  ...
  if (!device?.id) throw new Error('Missing Ancestry device state; run fam ancestry.session login again.');
  const http = new AncestryHttp();
