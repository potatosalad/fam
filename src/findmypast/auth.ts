...
export async function authenticateFindmypast(http = new FindmypastHttp()): Promise<FindmypastSession> {
  if (await readPrivateJson('findmypast/verification-required.json')) throw new Error('Findmypast requires browser verification. Run fam findmypast.session login --browser.');
  ...
  try { ({data} = await http.exchange<FindmypastTokens>(`${AUTH}/oauth/token`, {method: 'POST', body: loginBody(credentials)})); }
    ...
    if (error instanceof FindmypastHttpError && ['requires_verification', 'mfa_required', 'mfa_registration_required', 'too_many_attempts'].includes(error.code ?? '')) {
      ...
      throw new Error(`Findmypast ${error.code}; complete fam findmypast.session login --browser before another password login.`);
    }
    throw error;
  }
...
export async function refreshFindmypast(session: FindmypastSession, http = new FindmypastHttp()): Promise<FindmypastSession> {
  if (!session.tokens.refresh_token) throw new Error('No Findmypast refresh token; run fam findmypast.session login.');
...
export async function beginBrowserAuthorization(): Promise<{url: string; instructions: string}> {
  ...
  return {url: `${AUTH}/authorize?${params}`, instructions: 'Complete sign-in in your browser, then save the com.findmypast.prod:// callback URL to a private file and run fam findmypast.session login --callback-file FILE. The browser console may show the callback when no Android app is installed.'};
}
export function validateCallback(value: string, pending: PendingAuthorization): string {
  ...
  if (Date.now() - pending.createdAt > 30 * 60_000) throw new Error('Browser authorization expired; start fam findmypast.session login --browser again.');
...
export async function finishBrowserAuthorization(callback: string, http = new FindmypastHttp()) {
  ...
  if (!pending) throw new Error('Start fam findmypast.session login --browser first.');
