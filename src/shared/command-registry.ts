...
function add(provider: Command['provider'], legacy: string, objectAction: string, description: string,
  ...
  const flagList = [...(!['cli', 'internetarchive'].includes(provider) ? [flag('transport', {choices: ['auto','http','browser'], binding: false, description: 'Override automatic HTTP/browser transport for this command.'}), flag('browser-timeout', {type: 'integer', minimum: 0, maximum: 3600, binding: false, description: 'Override the wait for browser verification, in seconds.'})] : []), ...positional.map(name => flag(name.replace(/^\?/, ''), {required: !name.startsWith('?')})),
    ...
    ...['out', 'json', 'dry-run', 'fail-fast', 'errors', 'reasoning', 'help'].filter(name => !names.split(' ').includes(name) && !positionals.includes(name)).map(name => flag(name))];
...
for (const provider of authenticatedProviderNames) {
  ...
  if (!['geneanet', 'newspaperarchive', 'newspapers', 'americanancestors', 'fold3'].includes(provider)) add(provider, 'call', 'api call', provider === 'familysearch'
    ...
    provider === 'familysearch' ? ['operation'] : ['operation', '?input'], provider === 'familysearch' ? 'input query deceased' : provider === 'ancestry' ? 'query base' : ['myheritage', 'findmypast'].includes(provider) ? 'query' : '',
    {risk: api, flags: provider === 'familysearch' ? {deceased: {type: 'boolean', description: 'For persons.create only: add a Death conclusion if absent; requires evidence of death, not a guessed date.'}, input: {description: 'JSON input file, inline object, or - for stdin.'}, query: {multiple: true, description: 'Typed key=value query binding; repeatable.'}, 'dry-run': {description: 'Validate flags and operation JSON locally, including file/stdin input; no provider request.'}} : undefined});
...
add('familysearch','source-list','source list','List source titles, record ARKs, tags, and attachment/modification attribution in one command.', [], 'person-id max-pages', {flags:{'person-id':{required:true},'max-pages':{type:'integer',minimum:1,maximum:100,default:10,description:'Maximum history pages to inspect for attachment attribution; missing details remain null.'}}});
add('familysearch','merge-plan','merge plan','Preview exact facts, source references, and relationships proposed for a merge without changing the tree.', [], 'survivor-id duplicate-id reason include-vitals include-relationships include-analysis', {flags:{'survivor-id':{required:true},'duplicate-id':{required:true},reason:{required:true,description:'Evidence supporting the merge, written to attribution.changeMessage.'},'include-vitals':{type:'boolean',description:'Include unique duplicate vital conclusions; default preserves survivor vitals.'},'include-relationships':{type:'boolean',description:'Include duplicate relationships in the proposed copy lists.'},'include-analysis':{type:'boolean',description:'Also return the complete provider merge analysis.'},out:{description:'Save the prepared persons.merge input as a new private JSON file; inspect the printed plan before executing.'}}});
add('familysearch','attach-plan','record.attach plan','Prepare an attachment with selected native record facts copied in the same write.', [], 'person-id ark reason copy-fact', {flags:{'person-id':{required:true},ark:{required:true},reason:{required:true,description:'Why this indexed record identifies the tree person.'},'copy-fact':{multiple:true,description:'Native record fact type (for example Residence) or conclusion ID; repeat to select multiple facts. Omit to inspect candidates.'},out:{description:'Save prepared sources.attachRecord input as a new private JSON file.'}}});
add('familysearch','not-matches','hint.not-match list','Read not-a-match declarations with reasons, contributors, and modification dates.', [], 'person-id', {flags:{'person-id':{required:true}}});
add('familysearch','duplicate-details','hint get','Read a possible duplicate together with any not-a-match declaration for that pair.', [], 'person-id duplicate-id', {flags:{'person-id':{required:true},'duplicate-id':{required:true}}});
add('newspapers','publication-search','publication search','Search newspaper publication titles by keyword.', [], 'keyword limit offset', {flags:{keyword:{required:true},limit:{minimum:1,maximum:100,default:20},offset:{minimum:0,maximum:1000000,default:0}},pagination:'One page of title matches. Pass nextOffset with the same keyword.'});
...
add('fold3','publication-browse','publication browse','Navigate a publication’s hierarchy and list the scanned pages at the end of a path.', ['publication-id'], 'path prefix limit offset', {flags:{path:{multiple:true,description:'Exact returned browse value; repeat in order for every component of the path.'},prefix:{description:'Narrow the next branch by its label prefix.'},limit:{minimum:1,maximum:100,default:20}},pagination:'Branches report truncated; use --prefix to narrow them. Image lists return nextOffset. Copy an item’s entire path when descending.'});
add('fold3','file','file get','Identify the selected image’s file/scan cluster, title, and total page count.', ['?image-id'], 'entry-id');
add('fold3','file-images','file.image list','List pages in order within the selected file, with checked boundaries and page positions.', ['?image-id'], 'entry-id limit offset', {flags:{limit:{minimum:1,maximum:100,default:100}},pagination:'Pass nextOffset with the same image-id. Offset zero is the file’s first page, even when the anchor is in its middle.'});
add('fold3','file-download','file download','Export every page in the selected file with a resumable manifest and citation/checksum sidecars.', ['?image-id'], 'entry-id max-pages resume', {flags:{out:{required:true,description:'New destination directory, or the original export directory with --resume.'},'max-pages':{type:'integer',minimum:1,maximum:10000,default:500,description:'Refuse to start when the file exceeds this explicit page bound.'},resume:{type:'boolean',description:'Verify saved page identities and checksums, then download remaining pages.'}}});
...
add('fold3','image','image get','Read scan metadata, dimensions, and current viewing/download permissions.', ['?image-id'], 'entry-id');
...
add('fold3','download','image download','Save an authorized whole-image JPG export and citation/checksum sidecar without overwriting files.', ['?image-id'], 'entry-id allow-reduced', {flags:{'allow-reduced':{type:'boolean',description:'Explicitly accept an authorized reduced-resolution JPG; metadata records actual and source dimensions.'},out:{required:true,description:'Destination JPG file; also writes FILE.jpg.json with source citation and checksum.'}}});
...
add('familysearch', 'record collections', 'record collections', 'Find collection IDs and their required category types for the supplied record-search criteria.', [], `${fsRecordNames} ${fsRecordEvents} exact collection-id collection-type sort`, {flags: {...fsRecordFlags, sort: {choices:['count-desc','count-asc','title'], default:'count-desc', description:'Order matching collections; missing counts sort last.'}},
