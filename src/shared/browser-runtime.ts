...
export async function startBrowser(config?: BrowserConfig): Promise<unknown> {
  ...
  if (config.mode === 'local') {
    ...
    if (browserEngine(config) === 'cloakbrowser') {
      ...
      for (const name of ['Dockerfile', 'LICENSE.md', 'package.json', 'package-lock.json', 'launch.py', 'server.mjs', 'humanize.mjs', 'main.mjs']) hash.update(await readFile(join(browserDirectory, 'cloakbrowser', name)));
