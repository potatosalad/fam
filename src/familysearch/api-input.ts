...
export async function prepareApiInput(name: string, source?: string, query: string[] = []) {
  ...
  const input = operationQueryInput(name, value, query);
  return {input, ...prepareOperation(name as OperationName, input)};
}
