...
import {isResearchPath, isTranscriptPath} from './research-paths.js';
export {isResearchPath} from './research-paths.js';
      ...
      for (let redirect = 0; redirect < 6; redirect++) {
        ...
        try {
          response = await retryRead('familysearch', async () => {
            ...
            return external
              ? fetchWithBrowser('familysearch', current, {retryable:false, headers:{Accept:options.accept ?? 'image/*'}}, () => this.assets.fetch(current, {redirect:'manual', headers:{Accept:options.accept ?? 'image/*'}}), undefined, [], {readOnly:true})
              : current.origin === TRANSCRIPT_ORIGIN
              ? fetchWithBrowser('familysearch', current, {retryable:false, headers:{...credentials, Accept:options.accept ?? 'application/json'}}, () => this.assets.fetch(current, {redirect:'manual', headers:{...credentials, Accept:options.accept ?? 'application/json'}}), undefined, [], {readOnly:true})
