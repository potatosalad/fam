...
export class Fold3Error extends Error {
  constructor(readonly code: 'http' | 'session-rejected' | 'api-changed' | 'access-denied' | 'verification-required' | 'not-found' | 'ocr-unavailable' | 'reduced-resolution', readonly status?: number) {
    super(code === 'session-rejected' ? 'Fold3 sign-in is required. Run fam fold3.session login.'
      ...
      : code === 'reduced-resolution' ? 'Fold3 returned different dimensions from the full-size scan. No file was saved. Use --allow-reduced on image download to explicitly save an authorized reduced-resolution export.'
