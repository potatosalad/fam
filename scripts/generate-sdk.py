...
c = json.loads((ROOT / 'docs/familysearch/contracts.json').read_text())
discovery = json.loads((ROOT / 'docs/familysearch/discovery.json').read_text())
assert set(discovery['operations']) == {op['name'] for op in c['operations']}, 'Discovery must describe every implemented operation exactly once'
assert set(discovery['groups']) == {op['name'].split('.')[0] for op in c['operations']}, 'Discovery groups must match the operation inventory'
for name, info in discovery['operations'].items():
    assert info['description'].strip() and info['effect'] in ['read', 'write'], f'Missing description or effects: {name}'
...
outputs = {}
outputs['src/familysearch/generated/discovery.ts'] = '// Generated by scripts/generate-sdk.py from docs/familysearch/discovery.json. Do not edit.\n' + "import type { DiscoveryMetadata } from '../discovery-types.js';\nexport const discovery: DiscoveryMetadata = " + json.dumps(discovery, indent=2) + ';\n'
header = '// Generated by scripts/generate-sdk.py from docs/familysearch/contracts.json. Do not edit.\n'
...
for group in sorted({o['name'].split('.')[0] for o in c['operations']}):
    ...
    for o in c['operations']:
        ...
        doc += [f'| `{o["name"]}` | {discovery["operations"][o["name"]]["description"]} | {o["method"]} `{o["path"]}` | {inputs.replace(chr(124), chr(92)+chr(124))} | {output_type.replace(chr(124), chr(92)+chr(124))} |']
    doc += ['']
outputs['docs/familysearch/operations.md'] = '\n'.join(doc)
...
outputs['docs/familysearch/coverage.json'] = json.dumps(coverage,indent=2)+'\n'
...
outputs['docs/familysearch/coverage.md'] = '\n'.join(report)+'\n'
