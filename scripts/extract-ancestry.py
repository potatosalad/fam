...
Run after scripts/analyze-ancestry.sh. Generated JSON is also the SDK input.
"""
import argparse
import hashlib
import json
...
SMALI = ROOT / 'analysis/ancestry/smali'
...
def extract():
    ...
    return {'apkVersion': '18.16.3', 'rest': sorted(rest, key=lambda op: op['id']), 'graphql': sorted(graphql.values(), key=lambda op: op['name'])}
...
def main():
    ...
    contracts = extract()
    outputs = {ROOT / 'docs/ancestry/contracts.json': json.dumps(contracts, indent=2) + '\n',
               ROOT / 'src/ancestry/generated/contracts.ts': '// Generated by scripts/extract-ancestry.py. Do not edit.\nexport const contracts = ' + json.dumps(contracts, indent=2) + ' as const;\n'}
    ...
    print(f"Ancestry contracts: {len(contracts['rest'])} REST declarations, {len(contracts['graphql'])} GraphQL operations")
